...
\end{equation}
Equation (\ref{rho_SPDE}) is a stochastic generalization of the deterministic PDE (\ref{eq1}) from \S\ref{deterministic}, but describes the evolution of the distribution of breeding values instead of expressed characters. In \S\ref{wnc} we review the origins of this equation and provide some theory to help make sense of it, particularly the term $\dot W$.

We assume $\rho(g,t)$ is Gaussian which implies its mode coincides with $\bar x$. Furthermore, since $\sigma^2=G+\eta$, we can use equation (\ref{m_rel}) and the chain rule from calculus (see SM \S\ref{fit2fit}) to find
\begin{subequations}
\begin{equation}
...
## Regularity of deterministic solution \label{regular}

Here we show that solutions to the PDE described in the main text are twice continuously differentiable with respect to space and differentiable with respect to time. Our approach is an adaptation of that taken by Volpert [-@Volpert2014].

Recall that the PDE we consider is of the form
\begin{equation}\label{PDE_supp}
\dot\nu(x,t)=m(\nu,x)\nu(x,t)+\frac{\mu}{2}\Delta\nu(x,t),
\end{equation}
where $m(\nu,x)$ bounded above by $r\in\mathbb{R}$ is shorthand for $m((K\nu)(x,t),x)$ and $(K\nu)(x,t)=\int_\mathbb{R}\kappa(x,y)\nu(y,t)dy$ for some non-negative, bounded $\kappa$. Denoting $L^1(\mathbb{R})$ the Lebesgue space of integrable functions on $\mathbb{R}$ (ie, the set of functions $f:\mathbb{R}\to\mathbb{R}$ that satisfy $\|f\|_1=\int_\mathbb{R}|f(x)|dx<+\infty$ under the equivalence relation $f=g$ iff $\|f-g\|_1=0$), we assume $\nu(\cdot,0)\in L^1(\mathbb{R})$. Under these assumptions, we have already proved in SM \S\ref{finite} that $\nu(\cdot,t)\in L^1(\mathbb{R})$ for all $t\geq0$. Before proving the regularity of $\nu(x,t)$, we review the approach taken by Volpert [-@Volpert2014].

__Volperts approach__

In chapter 9 \S2.2 of Volpert [-@Volpert2014] complimentary conditions are assumed where $\kappa(x,y)=\varphi(x-y)$ for some integrable function $\varphi$ and initial condition $\nu(\cdot,0)\in L^\infty(\mathbb{R})$, where $L^\infty(\mathbb{R})$ is the Lebesgue space of bounded functions on $\mathbb{R}$ (ie, the set of functions $f:\mathbb{R}\to\mathbb{R}$ satisfying $\|f\|_\infty=\sup_{x\in\mathbb{R}}|f(x)|<+\infty$ under the equivalence relation $f=g$ iff $\|f-g\|_\infty=0$). Instead of the growth term $m(\nu,x)\nu(x,t)$ of PDE (\ref{PDE_supp}), Volpert worked with $f(\nu(x,t),(K\nu)(x,t))$ for some function $f:\mathbb{R}^2\to\mathbb{R}$ differentiable with respect to both arguments and satisfying, for some $q\geq0$,
\begin{equation}
f(x,y)\leq q(1+|x|+|y|), \ \forall \ x,y\in\mathbb{R}.
\end{equation}
Under these conditions, Volpert used semigroup theory to prove the solution $\nu(x,t)$ takes values in $L^\infty(\mathbb{R})$ for all $t\geq0$ and is differentiable with respect to time and twice continuously differentiable with respect to space. In particular, defining $\nu(t)$ as the function $x\to\nu(x,t)$ and keeping $\Phi(x,t)$ as defined in SM \S\ref{finite}, Volpert states the semigroup $\{S(t)\}_{t\geq0}$ given by 
\begin{equation}
S(t)\nu(0)=\int_0^t\frac{\mu}{2}\Delta\nu(x,s)ds=\int_\mathbb{R}\Phi(x-y,t)\nu(y,0)dy, \ \ \forall \ \ t\geq0, \ \nu(0)\in L^\infty(\mathbb{R})
\end{equation}
satisfies $\|S(t)\nu(0)\|_\infty\leq\|\nu(0)\|_\infty$ for all $t\geq0$ and $\nu(0)\in L^\infty(\mathbb{R})$. By setting
\begin{equation}
F(v)=f\left(v(x),\int_\mathbb{R}\varphi(x-y)v(y)dy\right), \ \ \forall \ \ v\in L^\infty(\mathbb{R}),
\end{equation}
Volpert notes that $F$ is locally Lipschitz continuous on $L^\infty(\mathbb{R})$ and in particular $\|F(v)\|_\infty\leq q\|v\|_\infty$. Hence, under Volperts conditions, solutions to PDE (\ref{PDE_supp}), given they exist, can be written as
\begin{equation}\label{volp_sol}
\nu(t)=S(t)\nu(0)+\int_0^tS(t-s)F(\nu(s))ds \ \ \forall \ \ t\geq0, \ \nu(0)\in L^\infty(\mathbb{R}).
\end{equation}
Volpert interprets the right-hand-side of equation (\ref{volp_sol}) as an operator $T$ acting on functions $v\in L^\infty(\mathbb{R}\times[0,\tau])$ with $\tau>0$. The space $L^\infty(\mathbb{R}\times[0,\tau])$ is given the norm $\|v\|_\infty^\infty=\max_{t\in[0,\tau]}\|v(t)\|_\infty$. The operator $T$ is claimed to have a single fixed point for sufficiently small $\tau$, which means that there exists exactly one function $v$ such that $Tv=v$. That is, there exists $v\in L^\infty(\mathbb{R}\times[0,\tau])$ such that
\begin{equation}
v(t)=(Tv)(t)=S(t)v(0)+\int_0^tS(t-s)(Fv)(s)ds\in L^\infty(\mathbb{R}) \ \ \forall \ \ t\in[0,\tau].
\end{equation}
Volpert merely states this is easy to conclude and the regularity conditions follow from it. We expand on why this is so. 

Combining the results $\|S(t)v(s)\|_\infty\leq\|v(s)\|_\infty$, $\|Fv(s)\|_\infty\leq q\|v(s)\|_\infty$ for all $t\geq0$, $s\in[0,\tau]$ and $v\in L^\infty(\mathbb{R}\times[0,\tau])$ with Minkowski's inequality, we have
\begin{equation}
\|Tv\|_\infty^\infty\leq\|v\|_\infty^\infty+q\tau\|v\|_\infty^\infty=(1+q\tau)\|v\|_\infty^\infty.
\end{equation}
Hence, for each $\tau>0$, $T$ is a contraction mapping on $L^\infty(\mathbb{R}\times[0,\tau])$. Banach's fixed point theorem therefore implies $T$ has exactly one fixed point. This holds for all $\tau>0$ which implies the existence and uniqueness of solutions to PDE (\ref{PDE_supp}) under Volpert's conditions. Hence, the solutions are differentiable with respect to time. Since $S(t)v$ is twice differentiable with respect to space for all $t\geq0$ and since $f$ is differentiable with respect to both arguments, $\int_0^tS(t-s)(Fv)(s)ds$ is twice differentiable with respect to space. Hence, solutions to PDE (\ref{PDE_supp}) are twice differentiable with respect to space.

__Adapting Volperts approach to our context__

To adapt Volperts approach to our situation, note that $\|S(t)\nu(0)\|_1=\|\nu(0)\|_1$ for all $\nu(0)\in L^1(\mathbb{R})$ and, setting
\begin{equation}
(Fv)(x)=m\left(\int_\mathbb{R}\kappa(x,y)v(y)dy,x\right)v(x), \ \ \forall \ \ v\in L^1(\mathbb{R}),
\end{equation}
we have $\|Fv\|_1\leq r\|v\|_1$ for all $v\in L^1(\mathbb{R})$. Hence, when they exist, solutions to PDE (\ref{PDE_supp}) under our assumptions can be written as
\begin{equation}
\nu(t)=S(t)\nu(0)+\int_0^tS(t-s)(F\nu(s))ds \ \ \forall \ \ t\geq0, \ \nu(0)\in L^1(\mathbb{R}).
\end{equation}
Then, using the same arguments employed by Volpert [-@Volpert2014], we conclude solutions to PDE (\ref{PDE_supp}) under our conditions exist, are unique and exhibit the desired regularity conditions.
...
Here we set out to show, given appropriate initial conditions, $N_\infty=\lim_{t\to\infty}N(t)=\tfrac{1}{c}(r-\tfrac{1}{2}\sqrt{a\mu})$, $\bar x_\infty=\lim_{t\to\infty}\bar x(t)=\theta$ and $\sigma^2_\infty=\lim_{t\to\infty}\sigma^2(t)=\sqrt{\tfrac{\mu}{a}}$ when
\begin{equation}\label{m_log_stab}
m(\nu,x)=r-\frac{a}{2}(\theta-x)^2-c\int_\mathbb{R}\nu(y,t)dy
\end{equation}
for $\theta\in\mathbb{R}$, $\mu\geq0$, $a,c>0$ and $r>\tfrac{1}{2}\sqrt{\mu a}$. When it exists, set $\nu_\infty(x)=\lim_{t\to\infty}\nu(x,t)$. The mean fitness can be calculated as
\begin{equation}
\bar m(t)=r-\frac{a}{2}((\theta-\bar x(t))^2-\sigma^2(t))-cN(t), \ \ \forall \ \ t\geq0.
\end{equation}
Solving for $N_\infty$ amounts to setting $\dot N(t)=0$ and solving for $N(t)$. This reduces to solving $\bar m(t)=0$ for $N(t)$, which returns
\begin{equation}
N_\infty=\frac{1}{c}\left(r-\frac{a}{2}\left((\theta-\bar x_\infty)^2-\sigma^2_\infty\right)\right).
\end{equation}
To find $\nu_\infty(x)$ we solve $\tfrac{\partial}{\partial t}\nu(x,t)=0$ for $\nu(x,t)$. This implies the following ordinary differential equation
\begin{equation}
\frac{d^2}{dx^2}\nu_\infty(x)=\left(\frac{2c}{\mu}N_\infty+\frac{a}{\mu}(\theta-x)^2-\frac{2r}{\mu}\right)\nu_\infty(x)
\end{equation}
which has the solution
\begin{equation}
\nu_\infty(x)=\frac{N_\infty}{\sqrt{2\pi}}\left(\frac{a}{\mu}\right)^{\frac{1}{4}}\exp\left(-\frac{(\theta-x)^2}{2\sqrt{\mu/a}}\right).
\end{equation}
From this expression we infer $\bar x_\infty=\theta$ and $\sigma^2_\infty=\sqrt{\mu/a}$. Hence $N_\infty=\frac{1}{c}\left(r-\frac{1}{2}\sqrt{a\mu}\right)$. This holds for any initial $\nu(\cdot,0)\in L^1(\mathbb{R})$ such that $\bar x(0)\in\mathbb{R}, \ \sigma^2(0),N(0)\in(0,+\infty)$.
