...
We shift our focus to showing mild solutions $\nu$ to PDE (\ref{PDE_supp}) are twice differentiable in $x$ and hence are classical solutions. Applying lemma 3.3.2 of Henry [-@Henry1981] we have that the mild solution $\nu$ is in fact a strong (but necessarily classical) solution to the Cauchy problem defined by PDE (\ref{PDE_supp}) with initial condition $\nu(0)=u$ for $t\in[0,T]$. In particular, this implies that $\nu(t)\in D(\Delta)$ for each $t\in(0,T)$ where $D(\Delta)=W^{2,1}(\mathbb{R})$ is the Sobolov space of integrable functions with weak derivatives at least of order $2$. Since $\frac{1}{p}\geq\frac{1}{p+1}\geq\frac{1}{p}-2$ for all $p\geq1$, the Sobolov embedding theorem [@Henry1981] implies $D(\Delta)\subset L^p(\mathbb{R})$, the Lebesgue space of $p$-integrable functions on $\mathbb{R}$, for all $p\geq1$. Hence $D(\Delta)\subset W^{2,p}(\mathbb{R})$, the Sobolov space of $p$-integrable functions with weak derivatives at least of order $2$, for all $p\geq1$. 
