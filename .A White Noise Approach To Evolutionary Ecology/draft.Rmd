...
Recall that $m(\nu,x)$ is shorthand for $m(\nu(x,t),x)$. That is, $m:[0,+\infty)\times\mathbb{R}\to\mathbb{R}$. Following our assumptions of the main text, we have that $m(y,x)$ is differentiable with respect to both $x$ and $y$ and there exists $r\in\mathbb{R}$ such that $m(y,x)\leq r$ across all $y\geq0$ and $x\in\mathbb{R}$. As in the main text, we also assume the initial condition $\nu(x,0)$ satisfies
\begin{equation}
N(0)=\int_{\mathbb{R}}\nu(x,0)dx<+\infty,
\end{equation}
\begin{equation}
...
Here we show that solutions to the PDE described in the main text are continuous in time and space, twice continuously differentiable with respect to space and differentiable with respect to time. Our approach is an adaptation of that taken by Volpert [-@Volpert2014].
...
\end{equation}
satisfies $\|S(t)\nu(0)\|_\infty\leq\|\nu(0)\|_\infty$ for all $t\geq0$ and $\nu(0)\in L^\infty(\mathbb{R})$. Then, by setting
\begin{equation}
F(v)=f\left(v(x),\int_\mathbb{R}\varphi(x-y)v(y)dy\right), \ \ \forall \ \ v\in L^\infty(\mathbb{R}),
\end{equation}
Volpert notes that $F$ is locally Lipschitz continuous on $L^\infty(\mathbb{R})$ and in particular $\|F(v)\|_\infty\leq q\|v\|_\infty$. Hence, under Volperts conditions, solutions to PDE (\ref{PDE_supp}) can be written as
\begin{equation}\label{volp_sol}
\nu(t)=S(t)\nu(0)+\int_0^tS(t-s)F(\nu(s))ds\in L^\infty(\mathbb{R}) \ \ \forall \ \ t\geq0, \ \nu(0)\in L^\infty(\mathbb{R}).
\end{equation}
Thanks to properties of the semigroup $\{S(t)\}_{t\geq0}$, solutions $\nu(t)$ given by (\ref{volp_sol}) satisfy the claimed regularity conditions.
...
\begin{equation}
F(v)=m\left(\int_\mathbb{R}\kappa(x,y)v(y)dy,x\right)v(x), \ \ \forall \ \ v\in L^1(\mathbb{R}),
\end{equation}
we have $\|F(v)\|_1\leq r\|v\|_1$ for all $v\in L^1(\mathbb{R})$. Hence, using the same arguments employed by Volpert [-@Volpert2014], we have that solutions to PDE (\ref{PDE_supp}) under our assumptions can be written as
\begin{equation}
\nu(t)=S(t)\nu(0)+\int_0^tS(t-s)F(\nu(s))ds\in L^1(\mathbb{R}) \ \ \forall \ \ t\geq0, \ \nu(0)\in L^1(\mathbb{R})
\end{equation}
which therefore satisfy the claimed regularity conditions.
...
\end{equation}
for $\theta\in\mathbb{R}$, $\mu\geq0$, $a,c>0$ and $r\geq\tfrac{1}{2}\sqrt{\mu a}$. Set $\nu_\infty(x)=\lim_{t\to\infty}\nu(x,t)$ and $p_\infty(x)=\nu_\infty(x)/N_\infty$, when these quantities exist. The mean fitness can be calculated as
\begin{equation}
